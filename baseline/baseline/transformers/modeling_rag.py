...
from transformers.file_utils import add_start_docstrings_to_callable, replace_return_docstrings
from transformers.modeling_auto import AutoModelForSeq2SeqLM, AutoModelForSequenceClassification
from transformers.modeling_bart import BartForConditionalGeneration
from transformers.modeling_dpr import DPRQuestionEncoder
from transformers.modeling_outputs import ModelOutput
from transformers.modeling_t5 import T5ForConditionalGeneration
from transformers.modeling_utils import PreTrainedModel
from .retrieval_rag import RagRetriever
from baseline.models import RobertaForSequenceEmbedding, BartForSequenceEmbedding
...
MODEL_FOR_SEQ_ENCODER_MAPPING = OrderedDict(
    [
        (RobertaConfig, RobertaForSequenceEmbedding),
        (BartConfig, BartForSequenceEmbedding),
...
class AutoModelForSeqEncoder:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if not isinstance(config, PretrainedConfig):
            if 'dpr' in pretrained_model_name_or_path:
                config, kwargs = DPRConfig.from_pretrained(
...
class RagModel(torch.nn.Module):
    ...
    def contextualize(self, input_ids, retriever, print_docs=False):
        ...
        doc_vectors, docs = retriever.retrieve(query_vectors.cpu().detach(), n_docs=self.n_docs)
    ...
    def forward(
        ...
        **generator_kwargs
        ...
        if encoder_outputs is not None:
            ...
            input_ids, attention_mask, doc_scores = self.contextualize(input_ids, retriever)
...
class PreTrainedRagModel(PreTrainedModel):
    ...
    def __init__(
        ...
        if question_encoder is None:
            question_encoder = AutoModelForSeqEncoder.from_pretrained(self.config.pretrained_question_encoder_name_or_path)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path=None, **kwargs):
        ...
        question_encoder = AutoModelForSeqEncoder.from_pretrained(
...
class RagForSequenceClassification(PreTrainedRagModel):

    base_model_prefix = "rag_seq_class"
